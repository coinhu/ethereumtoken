...
declare const alt: <A>(that: () => A[]) => (fa: A[]) => A[],
  ...
  partitionMap: <A, B, C>(f: (a: A) => Either<B, C>) => (fa: A[]) => Separated<B[], C[]>,
  partitionMapWithIndex: <A, B, C>(f: (i: number, a: A) => Either<B, C>) => (fa: A[]) => Separated<B[], C[]>,
  partitionWithIndex: {
    <A, B extends A>(refinementWithIndex: import('./FilterableWithIndex').RefinementWithIndex<number, A, B>): (
      fa: A[]
    ) => Separated<A[], B[]>
  ...
  separate: <A, B>(fa: Either<A, B>[]) => Separated<A[], B[]>
export {
  /**
   * @since 2.0.0
   */
