...
export function getApplyMonoid(M) {
    return Object.assign(Object.assign({}, getApplySemigroup(M)), { empty: right(M.empty) });
}
/**
...
export function getWitherable(M) {
    ...
    const filter = (ma, predicate) => isLeft(ma) ? ma : predicate(ma.right) ? ma : left(M.empty);
    const wither = (F) => {
...
export const either = {
    ...
    chainRec: (a, f) => tailRec(f(a), e => isLeft(e) ? right(left(e.left)) : isLeft(e.right) ? left(f(e.right.left)) : right(right(e.right.right))),
    throwError: left
};
const { alt, ap, apFirst, apSecond, bimap, chain, chainFirst, duplicate, extend, flatten, foldMap, map, mapLeft, reduce, reduceRight, fromOption, fromPredicate, filterOrElse } = pipeable(either);
export { 
/**
 * @since 2.0.0
 */
